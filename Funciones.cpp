...
estudiante::estudiante() //Constructor vac�o de la clase estudiante//
{
    identificador=0;
    codigo="";
    prom_gral=0;
...
estudiante::estudiante(int ident, string cod, int leng, int ing, int mat, int cien, int hist, int tecn, int art, int ed_fis) //Constructor con par�metros de la clase estudiante
{
    identificador = ident;
    codigo = cod;
...
string RemoveChar(string str, char c) //Funci�n que remueve un caracter de un string para eliminar los " que vienen en el archivo
{
   string result;
...
int str_to_int(string s) //Funci�n para transformar los n�meros con puntos en n�meros utilizables
{
...
void procesar(vekest &V) //Procedimiento principal del programa que procesa el archivo estudiante y lo transforma en un objeto estudiante
{
    ...
    if(archivo.is_open())
        {   string linea;
            ...
            {
                linea=RemoveChar(linea,'"'); //invocaci�n de funci�n que borra los "
                int iden ,i=0,notas[]={0,0,0,0,0,0,0,0};
                string codigo, segmento;
                stringstream help(linea);
                while(getline(help, segmento, ';' ))
                {
                    if(i==0) //Se guarda el n�mero identificatorio
                    {
                        char *cst = new char [segmento.length()+1];
                        strcpy(cst, segmento.c_str());
                        iden = atoi(cst);
                    }
                    if(i==1) //Se guarda el nombre gen�rico del alumno
                    {
                        codigo = segmento;
                    }
                    if(i>1) //Se guardan las notas en un arreglo para el acceso m�s f�cil
                    {
                        notas[i-2]=str_to_int(segmento); //Se invoca a la funci�n que transforma los n�meros con punto en n�meros usables
                    }
                    i++;
                }
                estudiante aux(iden,codigo,notas[0],notas[1],notas[2],notas[3],notas[4],notas[5],notas[6],notas[7]); //Creaci�n del estudiante con los datos recopilados
                V.push_back(aux); //Se a�ade el estudiante al vector
            }
        }
}
        ...
        for(int i = gap; i<n; i+=1)
        {
            ...
            case 0: //ordena respecto al promedio general
                for(j = i; j >= gap && V[j - gap].get_pg() > aux.get_pg(); j -= gap)
                    V[j] = V[j - gap];
                break;
            case 1: //ordena respecto al promedio art�stico
            ...
            case 2: //ordena respecto al promedio humanista
                for(j = i; j >= gap && V[j - gap].get_plehi() > aux.get_plehi(); j -= gap)
                    V[j] = V[j - gap];
                break;
            case 3: //ordena respecto al promedio t�cnico
...
void mejores(vekest &V, int indicador, string nombre_archivo) //Procedimiento que escribe el archivo dependiendo del indicador
{
    ...
    {
        int i=V.size();
        for(int j = i-100;j<i;j++) //Se escriben los �ltimos 100 estudiantes que por el ordenamiento son los 100 con promedio m�s alto
        {
            string aux ="";
        ...
        int a=0;
        while(a<100) //Se borran los �ltimos 100 estudiantes del vector para catalogar los siguientes
        {
            V.pop_back();
            a++;
        }
